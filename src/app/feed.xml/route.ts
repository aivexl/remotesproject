...
function extractTextFromContent(content: any): string {
  ...
  if (Array.isArray(content)) {
    return content
      ...
      .join(' ')
      .substring(0, 500); // Limit to 500 chars for RSS description
  }
  return '';
}
...
export async function GET() {
  try {
    ...
    const rssItems = sortedArticles.map((article) => {
      ...
      <category>${escapeXml(article.category === 'newsroom' ? 'News' : 'Academy')}</category>
...
<rss version="2.0" xmlns:content="http://purl.org/rss/1.0/modules/content/" xmlns:atom="http://www.w3.org/2005/Atom" xmlns:dc="http://purl.org/dc/elements/1.1/">
  <channel>
    <title>Beluga - Platform Crypto Indonesia Terdepan</title>
    <link>${baseUrl}</link>
    <description>Berita cryptocurrency, analisis market, dan edukasi blockchain terbaru dari Beluga. Platform terdepan untuk informasi crypto di Indonesia.</description>
