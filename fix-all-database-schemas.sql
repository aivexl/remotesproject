...
ADD COLUMN IF NOT EXISTS round_type VARCHAR(100);
...
-- Sample Exchanges (only insert if not exists)
...
)) AS new_data(name, country, region, founded, website_url, type, status, description, logo, trading_volume, pairs, features)
WHERE NOT EXISTS (SELECT 1 FROM crypto_exchanges WHERE crypto_exchanges.name = new_data.name);

-- Sample Airdrops (only insert if not exists)
...
)) AS new_data(project, token, network, status, reward_amount, total_allocation, min_allocation, max_allocation, 
  estimated_value, participants, requirements, start_date, end_date, website_url, logo, 
  type, category, description)
WHERE NOT EXISTS (SELECT 1 FROM crypto_airdrop WHERE crypto_airdrop.project = new_data.project);
